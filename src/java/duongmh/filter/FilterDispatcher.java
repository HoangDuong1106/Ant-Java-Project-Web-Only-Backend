...
public class FilterDispatcher implements Filter {
    ...
    public void doFilter(ServletRequest request, ServletResponse response,
        ...
        try {
            int firstResource = uri.lastIndexOf("/");
            int lastResource = uri.lastIndexOf("?");
            String resource = null;
            if (lastResource == -1) {
                resource = uri.substring(firstResource + 1);
            } else {
                resource = uri.substring(firstResource + 1, lastResource);
            }
            System.out.println(resource);
            if (resource.isEmpty()) {
                resource = Constraint.LOGIN_PAGE_LABEL;
            }
            // get servletContext and roadmap 
